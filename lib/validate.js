...
function validateProjectTelegramConfig(incoming) {
  ...
  const validModes = new Set(['full', 'ping', 'off']);
  const mode = typeof incoming.masterNotifyMode === 'string' ? incoming.masterNotifyMode.trim() : '';
  const sanitized = {
    enabled: typeof incoming.enabled === 'boolean' ? incoming.enabled : false,
    allowedUsers: [],
    masterNotifyMode: validModes.has(mode) ? mode : 'full',
  };
