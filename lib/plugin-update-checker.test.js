...
test('invalidatePluginUpdateCache clears cached plugin update results', async () => {
  await withPluginUpdateCheckerMocks({
    installed: [
      { key: 'gsd@get-shit-done', name: 'gsd', source: 'skill', isSkill: true },
    ],
    versions: { gsd: '1.0.0' },
    npmResponses: {
      'get-shit-done-cc': { version: '1.2.0' },
    },
  }, async ({ checkPluginUpdates, invalidatePluginUpdateCache }) => {
    const first = await checkPluginUpdates(true);
    const second = await checkPluginUpdates(false);
    assert.equal(second.cached, true);
    assert.equal(second.updates[0].latestVersion, '1.2.0');

    invalidatePluginUpdateCache();
    const third = await checkPluginUpdates(false);
    assert.equal(third.cached, false);
    assert.equal(third.updates[0].latestVersion, '1.2.0');
  });
});
