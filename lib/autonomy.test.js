...
test('autoAnswer loop dampening chooses stop for repeated /gsd-next prompts', () => {
  const engine = new AutonomyEngine({});
  const cfg = { autoAnswer: { fullAutonomy: true, selectRecommended: true } };
  const qd = {
    question: 'You’ve called /gsd-next 14 times consecutively. Continue automatic advancement now?',
    options: [{ label: 'Continue (Recommended)' }, { label: 'Stop here' }],
  };
  const result = engine.autoAnswer(qd, cfg);
  assert.equal(result.answer, '2');
  assert.match(result.reason, /loop dampening/i);
});
...
test('handleQuestion still auto-answers generic simple question when workflow detectors are inactive', () => {
  const mockWorkflows = {
    classifyQuestion() { return 'unknown'; },
    autoAnswer() { return null; },
  };
  const engine = new AutonomyEngine({ autoAnswer: { mode: 'full', fullAutonomy: true } }, mockWorkflows);
  const qd = { question: 'Continue? (y/n)' };
  const result = engine.handleQuestion('tab1', qd, null);
  assert.equal(result.action, 'auto-answer');
  assert.equal(result.answer, 'yes');
...
test('evaluatePermission auto-approves default safe tools when no rules configured', () => {
  const engine = new AutonomyEngine({});
  const result = engine.evaluatePermission({ toolName: 'Glob', input: {} });
  assert.equal(result.action, 'allow');
});

test('evaluatePermission denies tools in alwaysDeny list', () => {
  const engine = new AutonomyEngine({
    autonomy: { permissionRules: { alwaysDeny: ['NotebookEdit'] } }
  });
  const result = engine.evaluatePermission({ toolName: 'NotebookEdit', input: {} });
  assert.equal(result.action, 'deny');
});

test('evaluatePermission allows everything in full autonomy mode', () => {
  const engine = new AutonomyEngine({
    autoAnswer: { fullAutonomy: true }
  });
  const result = engine.evaluatePermission({ toolName: 'Agent', input: {} });
  assert.equal(result.action, 'allow');
  assert.ok(result.reason.includes('full autonomy'));
});

test('evaluatePermission asks user for unknown tool without full autonomy', () => {
  const engine = new AutonomyEngine({
    autoAnswer: { fullAutonomy: false }
  });
  const result = engine.evaluatePermission({ toolName: 'NotebookEdit', input: {} });
