...
test('validateProjectTelegramConfig accepts ping masterNotifyMode', () => {
  const r = validateProjectTelegramConfig({ enabled: true, masterNotifyMode: 'ping' });
  assert.equal(r.ok, true);
  assert.equal(r.config.masterNotifyMode, 'ping');
});
test('validateProjectTelegramConfig coerces invalid masterNotifyMode to full', () => {
  const r = validateProjectTelegramConfig({ enabled: true, masterNotifyMode: 'loud' });
  assert.equal(r.ok, true);
  assert.equal(r.config.masterNotifyMode, 'full');
});
