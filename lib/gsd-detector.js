...
class GsdDetector extends WorkflowDetector {
  ...
  detectDerailment(result, session) {
    ...
    if (result.numTurns > 1 && !this.detectAutoNext(result, session)) {
      ...
      // Don't trigger if output contains a /gsd command suggestion (detectAutoNext will handle it)
      const hasGsdSuggestion = /\/gsd[-:][\w-]+/i.test(lastChunk);
      // Don't trigger if a Skill tool was invoked (e.g., Skill: gsd-plan-phase) — session will continue naturally
      const hasSkillInvocation = /⚡\s*Skill\s+gsd|Skill.*gsd[-:]|skill:\s*"gsd/i.test(lastChunk);
      // Don't trigger if output indicates a clear stop point with explicit next-step instructions
      const hasExplicitNextStep = /Next Up|next step|▶ Next|then run|then:\s*\/gsd/i.test(lastChunk);
      // Don't trigger if output is a UAT/human verification checkpoint awaiting human input
      const isAwaitingHumanInput = /awaiting human|human.needed|human.?UAT|Reply with.*(?:number|1)|resume.*session|yes\s*\/\s*no\s*\/\s*skip\s*\/\s*blocked/i.test(lastChunk);
      if (!isQuestioning && !endsWithQuestion && !isNewProject && !isWaitingForAgents && !isPhaseInit && !isMilestoneComplete && !hasPhaseComplete && !alreadyDone && !askedQuestion && !hasGsdSuggestion && !hasSkillInvocation && !hasExplicitNextStep && !isAwaitingHumanInput) {
        return {
          prompt: '/gsd:next',
