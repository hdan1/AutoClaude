...
class GsdDetector extends WorkflowDetector {
  ...
  detectAutoNext(result, session) {
    ...
    if (/waiting (?:for|on).*(?:agent|research|task)|(?:agent|research).*(?:still|running|background)|once they.*(?:complete|finish|land|return)|hang tight|no action needed/i.test(text)) {
      return { prompt: 'continue', reason: 'Waiting for background agents', delaySecs: 15 };
    }
  ...
  detectDerailment(result, session) {
    ...
    if (result.numTurns > 1 && !this.detectAutoNext(result, session)) {
      ...
      const endsWithQuestion = /(?:what do you want|want me to|would you like|shall i|get started)\s*\??\s*$/i.test(text.trim());
      const isNewProject = /gsd:new-project|gsd:new-milestone/i.test(text);
      ...
      if (!isQuestioning && !endsWithQuestion && !isNewProject && !isWaitingForAgents && !isPhaseInit && !isMilestoneComplete && !hasPhaseComplete && !alreadyDone) {
        return {
          prompt: '/gsd:next',
