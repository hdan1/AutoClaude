...
class GsdDetector extends WorkflowDetector {
  ...
  detectDerailment(result, session) {
    ...
    if (result.numTurns > 1 && !this.detectAutoNext(result, session)) {
      ...
      if (!isQuestioning && !endsWithQuestion && !isNewProject && !isWaitingForAgents && !isPhaseInit && !isMilestoneComplete && !hasPhaseComplete && !alreadyDone && !askedQuestion && !hasGsdSuggestion && !hasSkillInvocation && !hasExplicitNextStep) {
        return {
          prompt: '/gsd:next',
