...
async function _checkClaudePlugin(plugin) {
  ...
  if (!resolvedUrl) {
    return { updateAvailable: false, error: `No manifest URL for marketplace: ${source}` };
  }
  ...
  if (!entry) return { updateAvailable: false, error: `Plugin ${plugin.name} not found in marketplace manifest` };

  const latestVersion = entry.version || '';
  const currentVersion = plugin.version || '';
  return { currentVersion, latestVersion, updateAvailable: _compareVersions(currentVersion, latestVersion) };
}

async function _checkNpmPlugin(plugin) {
  const versions = _readPluginVersions();
  const currentVersion = versions[plugin.name] || '';
  ...
  if (!npmPackage) {
    return { currentVersion, updateAvailable: false, error: `No npm package mapping for ${plugin.name}` };
  }
