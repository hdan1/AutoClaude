...
const DEFAULT_RECOMMENDED_PLUGINS = [
  ...
  { key: 'context7@context7-mcp',                       repo: 'upstash/context7', official: false, marketplace: 'context7-mcp', npmPackage: '@upstash/context7-mcp', installCmd: { win32: 'npx -y ctx7 setup --claude -y', darwin: 'npx -y ctx7 setup --claude -y', linux: 'npx -y ctx7 setup --claude -y' }, type: 'mcp', detectMcp: 'context7' },
...
async function detect() {
  logger.info('claude-detect', 'detect() called');
  ...
  logger.info('claude-detect', `findClaudePath => ${knownPath || 'null'}`);
  if (knownPath) {
    result.path = knownPath;
    result.installed = true;
    try {
      logger.info('claude-detect', `execFileAsync ${knownPath} --version`);
      ...
      logger.info('claude-detect', `version from known path: ${result.version}`);
    } catch (e) {
      const stderr = (e.stderr || '').toString();
      const stdout = (e.stdout || '').toString();
      result.version = extractVersion(stdout) || extractVersion(stderr);
      logger.warn('claude-detect', `known path --version failed: ${e.message}, extracted: ${result.version}`);
    }
  }

  // 2. Fallback: try `claude --version` on PATH
  if (!result.installed) {
    logger.info('claude-detect', 'trying claude on PATH');
    try {
      ...
      logger.info('claude-detect', `PATH claude version: ${result.version}`);
    } catch (e) {
      const stderr = (e.stderr || '').toString();
      const stdout = (e.stdout || '').toString();
      ...
      logger.info('claude-detect', `PATH claude failed: ${e.message}, ver=${ver||'null'}`);
    }
  }
  ...
  logger.info('claude-detect', `detect() done: installed=${result.installed}, version=${result.version}, auth=${result.authType}`);
  return result;
}
