...
class MasterTelegramBridge {
  ...
  forwardQuestion(tabInfo, questionPayload) {
    ...
    const modeRaw = typeof tabInfo?.masterNotifyMode === 'string' ? tabInfo.masterNotifyMode.trim().toLowerCase() : '';
    const mode = ['off', 'ping', 'full'].includes(modeRaw) ? modeRaw : 'full';
    if (mode === 'off') return;
    ...
    const projectBotUsernameRaw = typeof tabInfo?.projectBotUsername === 'string' ? tabInfo.projectBotUsername.trim() : '';
    const projectBotUsername = projectBotUsernameRaw.replace(/^@+/, '');
    const isValidProjectBotUsername = /^[A-Za-z0-9_]{5,32}$/.test(projectBotUsername);

    if (isValidProjectBotUsername) {
      const msg = mode === 'ping'
        ? `🔔 ${label} needs your input`
        : `❓ [${tabId} | ${alias} | ${label}] Question\n${questionText}`;
      const options = {
        reply_markup: {
          inline_keyboard: [[{ text: '💬 Open Project Bot', url: `https://t.me/${projectBotUsername}` }]],
        },
      };
      for (const chatId of this._allChatIds()) {
        this._send(chatId, msg, options);
      }
      return;
    }
