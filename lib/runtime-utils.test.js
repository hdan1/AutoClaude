...
test('buildCorruptBackupPath appends timestamped corrupt backup suffix', () => {
  const backupPath = buildCorruptBackupPath('C:/data/auto-claude.db', new Date('2026-04-11T09:08:07Z'));
  assert.match(backupPath, /auto-claude\.db\.corrupt-20260411-090807\.bak$/);
});
...
test('preserveCorruptDbFile renames corrupt DB file to backup path', () => {
  ...
  const result = preserveCorruptDbFile({
    ...
    now: new Date('2026-04-11T09:08:07Z'),
  });

  assert.equal(result.ok, true);
  assert.equal(moves.length, 1);
  assert.equal(moves[0].from, 'C:/data/auto-claude.db');
  assert.match(moves[0].to, /auto-claude\.db\.corrupt-20260411-090807\.bak$/);
});
