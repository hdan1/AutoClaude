...
function shouldRecover(result, model, config, recoveryCount) {
  ...
  // Need valid input tokens to measure (at least one token source must be present)
  const totalTokens = (result.inputTokens || 0) + (result.cacheReadTokens || 0) + (result.cacheCreateTokens || 0);
  if (totalTokens <= 0) {
  ...
  // Include all tokens that consume the context window (input + cache read + cache create)
  const totalInput = (result.inputTokens || 0) +
    (result.cacheReadTokens || 0) +
    (result.cacheCreateTokens || 0);
  const pct = hasTrustedInputTokens ? (totalInput / contextWindow) : 0;
...
/**
 * Build a structured state summary from session state for context recovery.
 * This is built app-side so recovery doesn't depend on Claude writing a file.
 * @param {object} sessionState - session.state
 * @param {object} result - last proxy.run() result
 * @returns {string} structured summary for the resume prompt
 */
function buildStateSummary(sessionState, result) {
  const parts = [];

  if (sessionState.gsdPhase) {
    parts.push(`GSD Phase: ${sessionState.gsdPhase}`);
  }
  if (sessionState.skillSource) {
    parts.push(`Workflow: ${sessionState.skillSource}`);
  }
  if (sessionState.lastAutoNextPrompt) {
    parts.push(`Last auto-next: ${sessionState.lastAutoNextPrompt}`);
  }

  const metrics = [];
  if (sessionState.totalInputTokens) metrics.push(`input: ${sessionState.totalInputTokens}`);
  if (sessionState.totalOutputTokens) metrics.push(`output: ${sessionState.totalOutputTokens}`);
  if (sessionState.totalCostUsd) metrics.push(`cost: $${sessionState.totalCostUsd.toFixed(4)}`);
  if (metrics.length) parts.push(`Session metrics: ${metrics.join(', ')}`);

  if (result?.toolCalls?.length) {
    const recent = result.toolCalls.slice(-5);
    const toolSummary = recent.map(tc => {
      const name = tc.tool || 'unknown';
      const err = tc.isError ? ' (ERROR)' : '';
      return `${name}${err}`;
    }).join(', ');
    parts.push(`Recent tools: ${toolSummary}`);
  }

  if (result?.fullText) {
    const tail = result.fullText.length > 300 ? result.fullText.slice(-300) : result.fullText;
    const lastLine = tail.split('\n').filter(l => l.trim()).pop() || '';
    if (lastLine) parts.push(`Last output: ${lastLine.substring(0, 150)}`);
  }
...
/**
 * Get the workflow-appropriate resume prompt with structured state context.
 * @param {object} sessionState - session.state with skillSource, gsdPhase
 * @param {string} [stateSummary] - optional structured summary from buildStateSummary
 * @returns {string} prompt to send as the first turn of the fresh session
 */
function getResumePrompt(sessionState, stateSummary) {
  let base;
  if (sessionState.skillSource === 'gsd' || sessionState.gsdPhase) {
    base = '/gsd-resume-work';
  } else {
    base = RESUME_PROMPT_GENERIC;
  }
