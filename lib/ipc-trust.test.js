...
test('withTrustedIpc returns custom default for untrusted event', async () => {
  const handler = withTrustedIpc('test', () => 'ok', {
    isTrusted: () => false,
  }, null);
  const result = await handler({});
...
test('withTrustedIpc returns custom object default for untrusted event', async () => {
  const handler = withTrustedIpc('test', () => 'ok', {
    isTrusted: () => false,
  }, { schema: {}, categories: [] });
  const result = await handler({});
  assert.deepStrictEqual(result, { schema: {}, categories: [] });
});
