...
test('detect returns null when gsdInstalled is false', () => {
  const d = new GsdDetector({
    gsd: { enabled: true },
    runtime: { workflowAvailability: { gsdInstalled: false } },
  });
  assert.equal(d.detect('/gsd:execute-phase 1'), null);
});

test('classifyQuestion returns unknown when gsdInstalled is false', () => {
  const d = new GsdDetector({
    gsd: { enabled: true },
    runtime: { workflowAvailability: { gsdInstalled: false } },
  });
  assert.equal(d.classifyQuestion({ question: 'Do you approve this plan?' }), 'unknown');
});

test('detectAutoNext returns null when gsdInstalled is false', () => {
  const d = new GsdDetector({
    gsd: { enabled: true, autoNext: true },
    runtime: { workflowAvailability: { gsdInstalled: false } },
  });
...
test('detectDerailment returns null when gsdInstalled is false', () => {
  const d = new GsdDetector({
    gsd: { enabled: true, derailmentCorrection: true },
    runtime: { workflowAvailability: { gsdInstalled: false } },
  });
...
// ── Human UAT gate detection (infinite loop fix) ──

test('detectDerailment: does not fire when output contains "awaiting human testing"', () => {
  const d = new GsdDetector();
  const result = makeResult('## Active UAT Sessions\n| # | Phase | Status |\n| 1 | 01-trip | partial |\nReply with 1 to resume this session.');
  const session = makeSession('executing phase 1');
  assert.equal(d.detectDerailment(result, session), null);
});

test('detectDerailment: does not fire when output contains "yes / no / skip / blocked"', () => {
  const d = new GsdDetector();
  const result = makeResult('### 1. Launch app with empty DB\nexpected: Startup routes to OPEN_CREATE_TRIP\nDid this match what you observed? (yes / no / skip / blocked)');
  const session = makeSession('executing phase 1');
  assert.equal(d.detectDerailment(result, session), null);
});

test('detectAutoNext: rejects suggestion when output shows human UAT gate', () => {
  const d = new GsdDetector();
  const result = makeResult('## Active UAT Sessions\n| 1 | 01-trip | partial | [awaiting human testing] | 0/3 |\nReply with 1 to resume.\nNext step: /gsd-verify-work');
  const session = makeSession('executing phase 1');
  assert.equal(d.detectAutoNext(result, session), null);
});
