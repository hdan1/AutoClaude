...
class SuperpowersDetector extends WorkflowDetector {
  ...
  _spCfg() {
    return this.config?.superpowers || {};
  }

  _isSuperpowersActive() {
    const spCfg = this._spCfg();
    if (spCfg.enabled === false) return false;
    const availability = this.config?.runtime?.workflowAvailability;
    if (availability && availability.superpowersInstalled === false) return false;
    return true;
  }

  detect(text) {
    if (!this._isSuperpowersActive()) return null;
  ...
  classifyQuestion(questionData) {
    if (!this._isSuperpowersActive()) return 'unknown';
  ...
  autoAnswer(questionData, config) {
    ...
    const spCfg = runtimeConfig?.superpowers || {};
    const availability = runtimeConfig?.runtime?.workflowAvailability;
    if (spCfg.enabled === false) return null;
    if (availability && availability.superpowersInstalled === false) return null;
  ...
  detectAutoNext(result, session) {
    ...
    const spCfg = this._spCfg();
    if (!this._isSuperpowersActive() || spCfg.autoChain === false) return null;
