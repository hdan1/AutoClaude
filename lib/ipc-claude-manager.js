...
function register(ipcMain, deps) {
  ...
  ipcMain.handle('toggle-claude-plugin', withTrustedIpc('toggle-claude-plugin', (event, { pluginKey, enabled }) => {
    ...
    if (result?.ok !== false) refreshWorkflowAvailability();
    if (result?.ok !== false && syncSettingForPlugin) syncSettingForPlugin(pluginKey, enabled);
    return result;
  }, trustDeps));
  ...
  const { invalidatePluginUpdateCache } = pluginUpdateChecker;
  ...
  ipcMain.handle('update-all-plugins', withTrustedIpc('update-all-plugins', async (event, { keys }) => {
    ...
    const hadSuccess = Array.isArray(results) && results.some((entry) => entry?.ok !== false);
    if (hadSuccess) {
      invalidatePluginUpdateCache();
      invalidateHealthCache();
      refreshWorkflowAvailability();
