...
function _createInstalledEntry(entry) {
  return { version: '', versionKnown: false, versionSource: 'unknown', ...entry };
}

function _applyVersionMeta(plugin, version, source) {
  const normalized = typeof version === 'string' ? version.trim() : '';
  if (normalized) {
    plugin.version = normalized;
    plugin.versionKnown = true;
    plugin.versionSource = source;
  } else {
    plugin.version = '';
    plugin.versionKnown = false;
    plugin.versionSource = 'unknown';
  }
  return plugin;
}

function _readRecordedPluginVersions() {
  const versionsPath = path.join(getClaudeHome(), 'plugin-versions.json');
  try {
    const parsed = JSON.parse(fs.readFileSync(versionsPath, 'utf8'));
    return parsed && typeof parsed === 'object' ? parsed : {};
  } catch (err) {
    if (err.code !== 'ENOENT') logger.debug('plugin-manager', `plugin-versions read failed: ${err.message}`);
    return {};
  }
}
...
function _readJsonFile(filePath) {
  try {
    ...
    if (err.code !== 'ENOENT') logger.debug('plugin-manager', `json read failed for ${filePath}: ${err.message}`);
    return null;
  }
}

function _looksLikeVersion(value) {
  return /^v?\d+(?:\.\d+)+(?:[-+][\w.-]+)?$/i.test(String(value || '').trim());
}

function _extractVersionFromCachePath(filePath) {
  const normalized = String(filePath).replace(/\\/g, '/');
  const parts = normalized.split('/').filter(Boolean);
  const pluginIdx = parts.lastIndexOf('.claude-plugin');
  const candidate = pluginIdx > 0 ? parts[pluginIdx - 1] : path.basename(path.dirname(filePath));
  if (_looksLikeVersion(candidate)) return candidate.replace(/^v/i, '');
  return '';
}

function _readCachedPluginMetadata(cacheDir, plugin) {
  const pluginDir = path.join(cacheDir, plugin.source, plugin.name);
  const candidates = [
  ...
  try {
    for (const entry of fs.readdirSync(pluginDir)) {
      candidates.push(path.join(pluginDir, entry, 'manifest.json'));
      candidates.push(path.join(pluginDir, entry, '.claude-plugin', 'plugin.json'));
    }
  } catch (err) {
    if (err.code !== 'ENOENT') logger.debug('plugin-manager', `plugin cache scan failed for ${plugin.name}: ${err.message}`);
  }

  let best = null;
  for (const candidate of candidates) {
    if (!fs.existsSync(candidate)) continue;
    const manifest = _readJsonFile(candidate);
    if (!manifest || typeof manifest !== 'object') continue;

    let mtimeMs = 0;
    try { mtimeMs = fs.statSync(candidate).mtimeMs || 0; } catch {}

    const version = typeof manifest.version === 'string' && manifest.version.trim()
      ? manifest.version.trim()
      : _extractVersionFromCachePath(candidate);
    const rank = version ? 1 : 0;

    if (!best || rank > best.rank || (rank === best.rank && mtimeMs > best.mtimeMs)) {
      best = {
...
function _readLatestCachedNpmVersion(packageName) {
  if (!packageName) return '';

  const packageSegments = packageName.split('/');
  let latest = null;

  for (const cacheRoot of _getNpmCacheRoots()) {
    try {
      for (const entry of fs.readdirSync(cacheRoot)) {
        const packageJsonPath = path.join(cacheRoot, entry, 'node_modules', ...packageSegments, 'package.json');
        if (!fs.existsSync(packageJsonPath)) continue;

        const parsed = _readJsonFile(packageJsonPath);
        if (!parsed || parsed.name !== packageName || typeof parsed.version !== 'string' || !parsed.version.trim()) continue;

        let mtimeMs = 0;
        try { mtimeMs = fs.statSync(packageJsonPath).mtimeMs || 0; } catch {}

        if (!latest || mtimeMs > latest.mtimeMs) {
          latest = { version: parsed.version.trim(), mtimeMs };
        }
      }
    } catch (err) {
      if (err.code !== 'ENOENT') logger.debug('plugin-manager', `npm cache scan failed for ${packageName}: ${err.message}`);
    }
  }
...
function _findRecommendedPluginForInstalled(plugin) {
  const name = (plugin.name || '').toLowerCase();
  const key = (plugin.key || '').toLowerCase();
  return DEFAULT_RECOMMENDED_PLUGINS.find((rec) => {
    const baseName = (rec.key || '').split('@')[0].toLowerCase();
    if (baseName === name) return true;
    if (key === `mcp:${baseName}`) return true;
    if ((plugin.isMcp || plugin.source === 'mcp-server') && rec.detectMcp) {
      return name.includes(String(rec.detectMcp).toLowerCase());
    }
    return false;
  }) || null;
}

function _applyRecordedVersion(plugin, recordedVersions) {
  const rec = _findRecommendedPluginForInstalled(plugin);
  const candidates = new Set();
  const name = (plugin.name || '').toLowerCase();
  const keyBase = (plugin.key || '').replace(/^mcp:/i, '').split('@')[0].toLowerCase();
  if (name) candidates.add(name);
  if (keyBase) candidates.add(keyBase);
  if (rec) candidates.add((rec.key || '').split('@')[0].toLowerCase());

  for (const candidate of candidates) {
    const version = recordedVersions[candidate];
    if (typeof version === 'string' && version.trim()) {
      return _applyVersionMeta(plugin, version, 'npm-recorded');
    }
  }
  return plugin;
}

function _inferLegacyVersion(plugin) {
  const rec = _findRecommendedPluginForInstalled(plugin);
  if (!rec) return null;

  const baseName = (rec.key || '').split('@')[0].toLowerCase();
  if (baseName === 'gsd') {
    const versionPath = path.join(getClaudeHome(), 'get-shit-done', 'VERSION');
    try {
      const version = String(fs.readFileSync(versionPath, 'utf8')).trim().replace(/^v/i, '');
      if (_looksLikeVersion(version)) return { version, source: 'version-file' };
    } catch (err) {
      if (err.code !== 'ENOENT') logger.debug('plugin-manager', `legacy GSD version read failed: ${err.message}`);
    }
  }

  if (rec.npmPackage) {
    const version = _readLatestCachedNpmVersion(rec.npmPackage);
    if (version) return { version, source: 'npm-cache' };
  }

  return null;
}

function _applyBestKnownVersion(plugin, recordedVersions) {
  _applyRecordedVersion(plugin, recordedVersions);
  if (plugin.versionKnown) return plugin;

  const inferred = _inferLegacyVersion(plugin);
  if (inferred && inferred.version) {
    return _applyVersionMeta(plugin, inferred.version, inferred.source);
  }

  return plugin;
}

function _recordInstalledVersion(baseName, npmPackage) {
  if (!baseName || !npmPackage) return;
  try {
    const { savePluginVersion } = require('./plugin-update-checker');
    const version = execFileSync('npm', ['view', npmPackage, 'version'], {
      timeout: 15000,
      windowsHide: true,
      encoding: 'utf8',
      shell: process.platform === 'win32',
    }).trim();
    if (version) savePluginVersion(baseName, version);
  } catch (err) {
    logger.debug('plugin-manager', `Could not record version for ${baseName}: ${err.message}`);
  }
}
...
function listPlugins() {
  ...
  const recordedVersions = _readRecordedPluginVersions();
  const installed = [];
  ...
  for (const [key, val] of Object.entries(enabled)) {
    ...
    installed.push(_createInstalledEntry({ key, name, source, enabled: !!val, community: isCommunity }));
  }

  // Try to read cached plugin info for descriptions and manifest-backed versions
  const cacheDir = path.join(getClaudeHome(), 'plugins', 'cache');
  for (const plugin of installed) {
    const metadata = _readCachedPluginMetadata(cacheDir, plugin);
    if (metadata) {
      plugin.description = metadata.description || plugin.description || '';
      _applyVersionMeta(plugin, metadata.version || '', 'manifest');
    }
    _applyBestKnownVersion(plugin, recordedVersions);

    const rec = _findRecommendedPluginForInstalled(plugin);
    if (rec && rec.type === 'mcp') plugin.isMcp = true;
    if (rec && rec.type === 'skill') plugin.isSkill = true;
  }
  ...
  for (const [serverName, serverConfig] of Object.entries(allMcpServers)) {
    ...
    const plugin = _createInstalledEntry({
      key: `mcp:${serverName}`,
      name: serverName,
      source: 'mcp-server',
...
function installContext7Directly() {
  ...
  try {
    ...
    _recordInstalledVersion('context7', '@upstash/context7-mcp');
...
function installViaCommand(recPlugin, key) {
  ...
  // GSD skill pack: use npm install -g with --ignore-scripts to avoid triggering
  // the package's prepublishOnly hook (build:hooks + build:sdk which does cd sdk && npm ci)
  // — that nested npm ci in the sdk/ subdirectory fails on some environments.
  // --ignore-scripts skips the build phase but still links the global binary correctly.
  if (baseName === 'gsd') {
    return installGsdViaNpmGlobal(recPlugin, key);
  }
...
// Install GSD via npm install -g --ignore-scripts instead of npx.
// The npx path runs the package's prepublishOnly which does cd sdk && npm ci --
// that nested npm ci fails on some environments (network/permissions/npm version).
// --ignore-scripts skips the build hooks + SDK build but still links the binary.
function installGsdViaNpmGlobal(recPlugin, key) {
  const pkg = recPlugin.npmPackage || 'get-shit-done-cc';
  return new Promise((resolve) => {
    let settled = false;
    const logger2 = require('./logger');
    logger2.info('plugin-manager', `Installing GSD via npm install -g --ignore-scripts ${pkg}`);

    const INSTALL_TIMEOUT_MS = 120000;
    const proc = spawn('npm', ['install', '-g', pkg, '--ignore-scripts'], {
      stdio: ['ignore', 'pipe', 'pipe'],
      windowsHide: true,
      shell: true,
    });
    ...
    const timer = setTimeout(() => {
      if (!settled) {
        settled = true;
        try { proc.kill(); } catch (e) { /* ignore */ }
        logger2.error('plugin-manager', `GSD install timed out after ${INSTALL_TIMEOUT_MS / 1000}s`);
        resolve({ ok: false, error: 'GSD install timed out', output });
      }
    }, INSTALL_TIMEOUT_MS);
    ...
    proc.on('close', code => {
      ...
      clearTimeout(timer);
      if (code === 0) {
        logger2.info('plugin-manager', `GSD installed successfully`);
        resolve({ ok: true, output });
      } else {
        logger2.error('plugin-manager', `GSD install failed with code ${code}`, output);
        resolve({ ok: false, error: `GSD install failed (code ${code})`, output });
      }
    });
    proc.on('error', err => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      logger2.error('plugin-manager', `GSD install spawn error`, err);
      resolve({ ok: false, error: `GSD install error: ${err.message}` });
    });
  });
}
...
function updatePlugin(key) {
  ...
  if (recPlugin && recPlugin.installCmd && recPlugin.installCmd[process.platform]) {
    ...
    return installViaCommand(recPlugin, key).then(result => {
      if (result.ok && recPlugin.npmPackage) {
        _recordInstalledVersion(baseName, recPlugin.npmPackage);
      }
      return result;
    });
  }
