...
function installViaCommand(recPlugin, key) {
  ...
  return new Promise((resolve) => {
    ...
    proc.stderr.on('data', d => { output += d.toString(); });
    proc.on('close', code => {
      // For context7, check if MCP was actually installed despite warnings
      ...
      if (baseName === 'context7') {
        try {
          const claudeJsonPath = path.join(os.homedir(), '.claude.json');
          const claudeJson = JSON.parse(fs.readFileSync(claudeJsonPath, 'utf8'));
          const mcps = claudeJson.mcpServers || {};
          context7Installed = Object.keys(mcps).some(k => k.toLowerCase().includes('context7'));
        } catch { context7Installed = false; }
      }
      // Use evaluateToolInstallResult for known tools (handles context7 symlink warnings, etc.)
      const verdict = evaluateToolInstallResult({ key: baseName, code, output, context7Installed });
      if (verdict.ok) {
        if (verdict.warning) {
