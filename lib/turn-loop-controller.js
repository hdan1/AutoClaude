...
const REPEATED_QUESTION_THRESHOLD = 3;
...
class TurnLoopController {
  ...
  /**
   * Track a question and detect if it's been asked repeatedly.
   * Returns { repeated: true, count, questionFingerprint } if threshold exceeded, null otherwise.
   */
  checkRepeatedQuestion(questionText) {
    if (!questionText) return null;
    const fp = this._fingerprint(questionText);
    this.questionHistory.push(fp);
    if (this.questionHistory.length > MAX_HISTORY) this.questionHistory.shift();

    const count = this.questionHistory.filter(h => h === fp).length;
    if (count >= REPEATED_QUESTION_THRESHOLD) {
      return {
        repeated: true,
        count,
        questionFingerprint: fp,
        reason: `Same question asked ${count} times — likely a confirmation gate loop`,
      };
    }
    return null;
  }
