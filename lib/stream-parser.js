...
function _parseAssistant(proxy, event, result, elapsed) {
  ...
  if (msg.usage) {
    ...
    // Only mark as untrusted when we encounter usage data in streaming assistant chunks.
    // The authoritative value is set in _parseResult from the final result block.
    if (result.hasTrustedInputTokens !== false) result.hasTrustedInputTokens = false;
    proxy.emit('metrics', { model: result.model, inputTokens: result.inputTokens, outputTokens: result.outputTokens });
  }
  if (msg.content) {
    for (const block of msg.content) {
