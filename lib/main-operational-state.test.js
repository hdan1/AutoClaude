...
function buildSendResponseHandler(deps) {
  const [, body] = extract(
    /ipcMain\.on\('send-response', withTrustedIpc\('send-response', \(event, data\) => \{([\s\S]*?)\n\}, trustDeps\)\);/,
    'send-response handler'
  );

  return new Function(
    'deps',
    `const { sessionManager, sendToTab, validateResponse } = deps; return (input = {}) => { const event = null; const data = input; ${body} };`
  )(deps);
}

function buildQuestionAnswerHandler(deps) {
  const [, body] = extract(
    /ipcMain\.on\('question-answer', withTrustedIpc\('question-answer', \(event, data\) => \{([\s\S]*?)\n\}, trustDeps\)\);/,
...
test('send-response logs a visible error when answering a stopped session', () => {
  const sent = [];
  let forwarded = false;
  const sendResponse = buildSendResponseHandler({
    sessionManager: {
      get: () => ({ state: { running: false } }),
      sendResponse: () => { forwarded = true; },
    },
    sendToTab: (tabId, channel, payload) => sent.push([tabId, channel, payload]),
    validateResponse: (value) => ({ valid: true, text: String(value).trim() }),
  });

  sendResponse({ tabId: 'tab-1', text: '42' });

  assert.equal(forwarded, false);
  assert.deepEqual(sent, [[
    'tab-1',
    'log',
    { type: 'stderr', text: 'Cannot send answer: session is not running.' },
  ]]);
});

test('question-answer logs a visible error when answering a stopped session', () => {
  const sent = [];
  let forwarded = false;
  const questionAnswer = buildQuestionAnswerHandler({
    sessionManager: {
      get: () => ({ state: { running: false } }),
      sendResponse: () => { forwarded = true; },
    },
  ...
  questionAnswer({ tabId: 'tab-1', text: '42' });
