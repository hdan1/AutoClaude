...
function isExpectedAutoUpdateFeed404(errorLike) {
  const raw = String(errorLike?.message || errorLike || '');
  return /releases\.atom/i.test(raw) && /\b404\b/.test(raw);
}

function summarizeAutoUpdateError(errorLike, maxLen = 240) {
  const raw = String(errorLike?.message || errorLike || '').trim();
  if (!raw) return 'Unknown updater error';

  const beforeHeaders = raw.split(/Headers:/i, 1)[0];
  let compact = beforeHeaders
    .replace(/[\r\n]+/g, ' ')
    .replace(/\s{2,}/g, ' ')
    .replace(/^['"]+|['"]+$/g, '')
    .trim();

  if (!compact) compact = 'Unknown updater error';
  if (compact.length > maxLen) compact = compact.slice(0, maxLen - 1) + '…';
  return compact;
}
