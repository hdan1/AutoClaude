    ...
    if (questionData) {
      ...
      if (qList.length > 0) {
        ...
        if (q.options && Array.isArray(q.options)) {
          ...
          // Loop dampening: if /gsd-next has been called many times consecutively,
          // prefer the explicit stop option to break unattended auto-looping.
          const qTextLower = qText.toLowerCase();
          const gsdNextLoopMatch = qTextLower.match(/\/gsd-next\s+(\d+)\s+times\s+consecutively/);
          const gsdNextCount = gsdNextLoopMatch ? parseInt(gsdNextLoopMatch[1], 10) : NaN;
          const stopIdx = optLabels.findIndex(l => /\bstop\s+here\b/i.test(l));
          const stopAt = cfg.autoAnswer.gsdNextConsecutiveStopAt || 12;
          if (cfg.autoAnswer.fullAutonomy && stopIdx >= 0 && Number.isFinite(gsdNextCount) && gsdNextCount >= stopAt) {
            return {
              answer: String(stopIdx + 1),
              reason: `loop dampening: /gsd-next called ${gsdNextCount}x consecutively`,
            };
          }
        ...
        // freeText flag signals review mode to use longer countdown since user must read + type
        if (cfg.autoAnswer.fullAutonomy && qText) {
          if (/\(y\/n\)|\[y\/n\]|shall i|should i|do you want|proceed\?|continue\?|approve|confirm|ready to|go ahead/i.test(qText)) {
            return { answer: 'yes', reason: 'auto-confirmed (y/n pattern)', freeText: true };
          }
          if (/approve.*plan|review.*plan|does.*plan.*look|is.*plan.*ready|ready to implement/i.test(qText)) {
            return { answer: 'yes, approved - proceed with implementation', reason: 'auto-approved plan', freeText: true };
          }
          if (/which.*prefer|what.*would you|how.*should|pick|choose|select|what.*name|what.*approach/i.test(qText)) {
            return { answer: 'you decide - pick the best option based on your expertise', reason: 'auto-delegated choice', freeText: true };
          }
        }
      }
    }
    ...
    if (tier === 'simple') {
      ...
      if (result) {
        ...
        if (result.noRecommended) {
          ...
          // But only skip review if mode is actually 'full' (not 'review')
          if (mode === 'full' && cfg.autoAnswer.skipReviewInFullAutonomy !== false && cfg.autoAnswer.fullAutonomy && !hasTelegram) {
        ...
        if (mode === 'review') {
          // Free-text questions need longer countdown — user must read context and type
          const countdown = result.freeText
            ? (cfg.autoAnswer.noRecommendedTimeoutSeconds || 30)
            : (cfg.autoAnswer.reviewCountdownSeconds || 10);
          return { action: 'review', answer: result.answer, reason: result.reason, countdown };
        }
    ...
    if (tier === 'critical' || tier === 'unknown') {
      ...
      if (cfg.autoAnswer.fullAutonomy) {
        ...
        if (result) {
          ...
          if (result.noRecommended) {
            // Full autonomy + no Telegram → skip review (only in 'full' mode, not 'review')
            if (mode === 'full' && cfg.autoAnswer.skipReviewInFullAutonomy !== false && !hasTelegram) {
